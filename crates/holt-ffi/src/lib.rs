...
///
/// The handle replays durable WAL records into memory but never
/// changes the store. Mutations and checkpoints return `HOLT_ERR`.
///
/// # Safety
///
/// `path` must be a non-null NUL-terminated UTF-8 string. `out`
...
mod tests {
    ...
    #[test]
    fn read_only_tree_reads_and_rejects_writes() {
        unsafe {
            let dir = tempdir().unwrap();
            ...
            let mut blocked_writer = ptr::null_mut();
            assert_eq!(
                holt_tree_open_with_wal_sync(path.as_ptr(), 0, &mut blocked_writer),
                HOLT_ERR
            );
            let msg = CStr::from_ptr(holt_last_error_message()).to_str().unwrap();
            assert!(msg.contains("incompatible access mode"));
